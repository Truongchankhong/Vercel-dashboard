    ...
    <tr>
      ...
      <td class="px-4 py-2 border">
        <div class="flex items-center gap-2">
          <button onclick="handleConfirmation('${row.rpro}', 'Có liệu')" class="px-3 py-1 bg-green-500 text-white rounded text-sm hover:bg-green-600 ${row.confirm === 'Có liệu' ? 'ring-2 ring-offset-1 ring-green-500 shadow-inner brightness-90' : ''}">
            Có liệu
          </button>
          <button onclick="handleConfirmation('${row.rpro}', 'Không có liệu')" class="px-3 py-1 bg-gray-600 text-white rounded text-sm hover:bg-gray-700 ${row.confirm === 'Không có liệu' ? 'ring-2 ring-offset-1 ring-gray-600 shadow-inner brightness-90' : ''}">
            Không có liệu
          </button>
...
window.handleConfirmation = async (rpro, status) => {
    ...
    if (error) {
        ...
        if (savedEl) {
            ...
            setTimeout(() => {
                loadConfirmList(); // Reload to update button styling
            }, 500);
        }
    }
};
